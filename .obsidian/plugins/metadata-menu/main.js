...
// src/fileClass/OptionsMultiSelectModal.ts
...
var getFiles = (plugin, field, dvQueryString, currentFile) => {
  const getResults = (api) => {
    try {
      ...
      new import_obsidian13.Notice(`Wrong query for field <${field.name}>
...
var AbstractFileBasedField = class extends FieldManager2 {
  ...
  createFileContainer(container) {
    ...
    this.dvQueryString = new import_obsidian13.TextAreaComponent(dvQueryStringContainer);
    ...
    const customRendering = new import_obsidian13.TextAreaComponent(customeRenderingContainer);
    ...
    const customSorting = new import_obsidian13.TextAreaComponent(customSortingContainer);
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian13.setIcon)(searchBtn, FieldIcon["File" /* File */]);
...
var FileFuzzySuggester = class extends import_obsidian14.FuzzySuggestModal {
  constructor(plugin, file, field, eF, indexedPath, lineNumber = -1, asList = false, asBlockquote = false, previousModal) {
    ...
    if (link) {
      ...
      if (file2 instanceof import_obsidian14.TFile)
  ...
  renderSuggestion(value, el) {
    ...
    if (((_b = this.selectedFile) == null ? void 0 : _b.path) === value.item.path) {
      ...
      const iconContainer = el.createDiv({ cls: "icon-container" });
      (0, import_obsidian14.setIcon)(iconContainer, "check-circle");
    }
    this.inputEl.focus();
  }
...
var OptionsMultiSelectModal = class extends import_obsidian15.SuggestModal {
  constructor(plugin, fileClassFile, field, parentFieldSet) {
    ...
    this.input = this.parentFieldSet.filters[id];
  ...
  buildFooterActions(footerActionsContainer) {
    ...
    const cancelButton = new import_obsidian15.ButtonComponent(footerActionsContainer);
    cancelButton.setIcon("cross");
    cancelButton.onClick(() => this.close());
    cancelButton.setTooltip("Cancel");
    const clearButton = new import_obsidian15.ButtonComponent(footerActionsContainer);
  ...
  buildConfirm(footerActionsContainer) {
    ...
    infoContainer.setText("Alt+Enter to save");
    const confirmButton = new import_obsidian15.ButtonComponent(footerActionsContainer);
    confirmButton.setIcon("checkmark");
    confirmButton.onClick(async () => {
      await this.replaceValues();
      this.close();
  ...
  renderSuggestion(value, el) {
    ...
    if (Object.keys(fieldStates).includes(value)) {
      (0, import_obsidian15.setIcon)(icon, displayIcon[value]);
    }
...
var Field = class {
  ...
  buildColumnMoverBtn(component) {
    const buildBtn = (direction) => {
      const btn = new import_obsidian16.ButtonComponent(component);
  ...
  buildVisibilityBtn(component) {
    this.visibilityButton = new import_obsidian16.ButtonComponent(component).setIcon(this.isColumnHidden ? "eye-off" : "eye").onClick(() => this.setVisibilityButtonState(!this.isColumnHidden));
  }
  ...
  buildFilter(container, parentFieldSet, name, debounced) {
    var _a;
    const fieldFilterContainer = container.createDiv({ cls: "filter-input" });
    const filter = new import_obsidian16.TextComponent(fieldFilterContainer);
    if (name === "file") {
      filter.setValue("");
      filter.onChange((value) => {
        parentFieldSet.filters[name].inputEl.value = value;
      ...
      (_a = filter.inputEl.parentElement) == null ? void 0 : _a.appendChild(button);
      (0, import_obsidian16.setIcon)(button, "chevron-down");
      filter.setValue("");
      filter.onChange((value) => {
        parentFieldSet.filters[name].inputEl.value = value;
        debounced(parentFieldSet);
      ...
      button.onclick = () => {
        ...
        const field = (_a2 = plugin.fieldIndex.fileClassesFields.get(fileClass.name)) == null ? void 0 : _a2.find((f) => f.isRoot() && f.name === this.name);
  ...
  buildFilterComponent() {
    ...
    this.buildFilter(this.container, this.parentFieldSet, (field == null ? void 0 : field.name) || "file", debounced);
  }
};
var FieldSet = class {
  ...
  resetFilters() {
    Object.keys(this.filters).forEach((name) => {
      ...
      if (filter instanceof import_obsidian16.DropdownComponent)
        filter.selectEl.value = "all";
      else
  ...
  changeView(_name) {
    ...
    if (_name && savedViews.find((view) => view.name === _name)) {
      ...
      Object.keys(this.filters).forEach((name) => {
        var _a2, _b;
        const filter = this.filters[name];
        if (filter instanceof import_obsidian16.TextComponent) {
          filter.inputEl.value = ((_a2 = savedView == null ? void 0 : savedView.filters.find((f) => f.name === name)) == null ? void 0 : _a2.query) || "";
        } else if (filter instanceof import_obsidian16.DropdownComponent) {
          filter.selectEl.value = ((_b = savedView == null ? void 0 : savedView.filters.find((f) => f.name === name)) == null ? void 0 : _b.query) || "all";
        }
      });
...
var SavedView = class {
  ...
  buildFilters(filters) {
    Object.entries(filters).forEach(([name, query]) => {
      this.filters.push({
        name,
        query: query instanceof import_obsidian17.TextComponent ? query.inputEl.value : query.selectEl.value
      });
    });
  }
...
var CreateSavedViewModal = class extends import_obsidian17.Modal {
  ...
  buildModal() {
    ...
    const nameInput = new import_obsidian17.TextComponent(nameContainer);
    nameInput.inputEl.addClass("with-label");
    nameInput.inputEl.addClass("full-width");
    ...
    const infoContainer = actionsContainer.createDiv({ cls: "info" });
    infoContainer.setText("Alt+Enter to save");
    const saveBtn = new import_obsidian17.ButtonComponent(actionsContainer);
    saveBtn.setDisabled(true);
    saveBtn.setIcon("file-plus-2");
    nameErrorContainer.hide();
    nameInput.onChange(async (value) => {
...
var FileClassTableView = class {
  ...
  */
  buildLimitManager(container) {
    ...
    const limitInput = new import_obsidian18.TextComponent(container);
    limitInput.setValue(`${this.limit}`);
    const debounced = (0, import_obsidian18.debounce)((fieldset) => fieldset.tableView.udpate(), 1e3, true);
  ...
  */
  ...
  buildViewSelector() {
    ...
    const options2 = this.fileClass.getFileClassOptions();
    const savedViews = options2.savedViews || [];
    this.viewSelect = new import_obsidian18.DropdownComponent(this.viewSelectContainer);
  ...
  buildSavedViewRemoveButton(container) {
    ...
    this.viewRemoveBtn = new import_obsidian18.ButtonComponent(btnContainer).setIcon("trash").setClass("remove-button").setDisabled(!this.selectedView).setTooltip(`Remove ${this.selectedView} view from the saved views`).onClick(async () => {
      var _a;
      const options2 = this.fileClass.getFileClassOptions();
  ...
  buildFavoriteViewManager(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    this.favoriteBtn = new import_obsidian18.ButtonComponent(btnContainer);
  ...
  buildCleanFields(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    const cleanFilterBtn = new import_obsidian18.ButtonComponent(btnContainer);
  ...
  buildSaveView(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    const saveViewBtn = new import_obsidian18.ButtonComponent(btnContainer);
  ...
  buildHideFilters(container) {
    const btnContainer = container.createDiv({ cls: "cell" });
    const hideFilterBtn = new import_obsidian18.ButtonComponent(btnContainer);
...
`;
            ...
            const fCField = fieldName !== "file" ? (_a2 = this.plugin.fieldIndex.fileClassesFields.get(this.fileClass.name)) == null ? void 0 : _a2.find((f) => f.name === fieldName) : void 0;
...
var FileClassView = class extends import_obsidian19.ItemView {
...
var FileClassChoiceModal = class extends import_obsidian20.SuggestModal {
...
// src/components/fileClassManager.ts
var FileClassManager = class extends import_obsidian21.Component {
...
var FileClassOptionsList = class {
  ...
  createExtraOptionList(openAfterCreate = true) {
    ...
    let addMissingFieldsAction = () => {
      new import_obsidian22.Notice("Something went wrong, please check your fileClass definitions");
    };
    if (this.fromFile) {
      const dvApi = (_a = this.plugin.app.plugins.plugins.dataview) == null ? void 0 : _a.api;
      if (dvApi) {
...
var OptionsList = class {
  ...
  createAndOpenFieldModal(node) {
    ...
    if (managedField) {
      ...
      new import_obsidian23.Notice("No field with definition at this position", 2e3);
    }
  }
  ...
  buildFieldOptionsForMenu() {
    if (isMenu2(this.location)) {
      ...
      const view = this.plugin.app.workspace.getActiveViewOfType(import_obsidian23.MarkdownView);
      if (view == null ? void 0 : view.editor) {
        const action = async () => {
          if (!view.file || !(view.file instanceof import_obsidian23.TFile))
            return;
  ...
  addFieldAtCurrentPositionOption() {
    var _a;
    const currentView = this.plugin.app.workspace.getActiveViewOfType(import_obsidian23.MarkdownView);
...
var ObjectField = class extends FieldManager2 {
  ...
  createDvField(dv, p, fieldContainer, attrs) {
    ...
    (0, import_obsidian24.setIcon)(editBtn, FieldIcon[this.field.type]);
    editBtn.onclick = async () => {
      const file = this.plugin.app.vault.getAbstractFileByPath(p["file"]["path"]);
      const _eF = file instanceof import_obsidian24.TFile && await Note.getExistingFieldForIndexedPath(this.plugin, file, this.field.id);
      if (_eF)
...
var BaseSuggestModal = class extends import_obsidian25.SuggestModal {
  ...
  buildBackButton(container) {
    const backButton = new import_obsidian25.ButtonComponent(container);
...
// src/modals/BaseModal.ts
var BaseModal = class extends import_obsidian26.Modal {
  ...
  buildSimpleSaveBtn(fieldContainer) {
    ...
    infoContainer.setText("Alt+Enter to save");
    const saveBtn = new import_obsidian26.ButtonComponent(fieldContainer);
...
in <${this.field.name}> ${this.field.fileClassName ? this.field.fileClassName : "Metadata Menu"} settings`;
              if (errorName === "SyntaxError")
                new import_obsidian27.Notice(notice, 5e3);
            }
          } else {
  ...
  buildTemplateInputItem(fieldContainer, name) {
    ...
    const input = new import_obsidian27.TextComponent(fieldContainer);
    input.inputEl.addClass("with-label");
    input.inputEl.addClass("full-width");
  ...
  buildTemplateSelectItem(fieldContainer, name, options2) {
    ...
    fieldContainer.createDiv({ cls: "spacer" });
    const selectEl = new import_obsidian27.DropdownComponent(fieldContainer);
  ...
  buildResultPreview(fieldContainer) {
    this.renderedValue = new import_obsidian27.TextAreaComponent(fieldContainer);
  ...
  buildInputEl(container) {
    const inputEl = new import_obsidian27.TextAreaComponent(container);
...
var InputField = class extends FieldManager2 {
  ...
  createSettingContainer(container, plugin) {
    ...
    const templateValue = new import_obsidian28.TextAreaComponent(templateContainer);
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    if ((_a = attrs.options) == null ? void 0 : _a.alwaysOn)
      spacer.hide();
    (0, import_obsidian28.setIcon)(editBtn, FieldIcon["Input" /* Input */]);
    ...
    (0, import_obsidian28.setIcon)(validateIcon, "checkmark");
    ...
    (0, import_obsidian28.setIcon)(cancelIcon, "cross");
...
var BooleanModal = class extends BaseModal {
  ...
  buildToggleEl() {
    ...
    const trueButton = new import_obsidian29.ButtonComponent(choicesContainer);
    trueButton.setButtonText("True");
    trueButton.setClass("left");
    choicesContainer.createDiv({ cls: "spacer" });
    const falseButton = new import_obsidian29.ButtonComponent(choicesContainer);
...
var NumberModal = class extends BaseModal {
  ...
  buildInputEl() {
    ...
    this.numberInput = new import_obsidian30.TextComponent(fieldContainer);
    const numberInput = this.numberInput;
    numberInput.inputEl.focus();
    numberInput.setValue(`${this.value || ""}`);
    const minusBtn = new import_obsidian30.ButtonComponent(fieldContainer);
    minusBtn.setButtonText(`- ${!!step ? step : 1}`);
    minusBtn.setDisabled(!this.fieldManager.canDecrement(numberInput.getValue()));
    const plusBtn = new import_obsidian30.ButtonComponent(fieldContainer);
...
var NumberField = class extends FieldManager2 {
  ...
  createNumberContainer(container) {
    ...
    this.numberStepValue = new import_obsidian31.TextComponent(numberStepValueContainer);
    ...
    this.numberMinValue = new import_obsidian31.TextComponent(numberMinValueContainer);
    ...
    this.numberMaxValue = new import_obsidian31.TextComponent(numberMaxValueContainer);
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian31.setIcon)(editButton, FieldIcon["Number" /* Number */]);
    const decrementButton = fieldContainer.createEl("button");
    (0, import_obsidian31.setIcon)(decrementButton, "left-arrow");
    const incrementButton = fieldContainer.createEl("button");
    (0, import_obsidian31.setIcon)(incrementButton, "right-arrow");
    if (!((_a = attrs == null ? void 0 : attrs.options) == null ? void 0 : _a.alwaysOn)) {
      editButton.hide();
    ...
    (0, import_obsidian31.setIcon)(validateIcon, "checkmark");
    ...
    (0, import_obsidian31.setIcon)(cancelIcon, "cross");
    cancelIcon.onclick = (e) => {
      fieldContainer.removeChild(inputContainer);
      fieldContainer.appendChild(fieldValue);
...
// src/modals/BaseSelectModal.ts
var BaseSelecttModal = class extends import_obsidian32.SuggestModal {
  constructor(plugin, file, field, eF, indexedPath, lineNumber = -1, asList = false, asBlockquote = false, previousModal) {
    super(plugin.app);
    this.plugin = plugin;
    this.file = file;
  ...
  buildAddButton(container) {
    this.addButton = new import_obsidian32.ButtonComponent(container);
  ...
  buildFooterActions(footerActionsContainer) {
    footerActionsContainer.createDiv({ cls: "spacer" });
    this.buildConfirm(footerActionsContainer);
    const cancelButton = new import_obsidian32.ButtonComponent(footerActionsContainer);
    cancelButton.setIcon("cross");
    cancelButton.onClick(() => this.close());
    cancelButton.setTooltip("Cancel");
    const clearButton = new import_obsidian32.ButtonComponent(footerActionsContainer);
    clearButton.setIcon("eraser");
    clearButton.setTooltip("Clear field's value(s)");
    clearButton.onClick(async () => {
    ...
    if (fileClassName) {
      ...
      if (fileClass && fileClassAttribute) {
        ...
        if (Array.isArray(fileClassAttribute.options)) {
          ...
          if (valuesFile instanceof import_obsidian32.TFile && valuesFile.extension == "md") {
...
var TextInputSuggest = class {
  constructor(inputEl) {
    ...
    this.scope = new import_obsidian33.Scope();
...
var FileSuggest = class extends TextInputSuggest {
  ...
  getSuggestions(input_str) {
    ...
    all_files.forEach((file) => {
      if (file instanceof import_obsidian34.TFile && file.extension === this.extenstion && file.path.toLowerCase().contains(lower_input_str)) {
        files.push(file);
      }
    });
    return files;
...
var AbstractListBasedField = class extends FieldManager2 {
  ...
  createListNotePathContainer(container, plugin) {
    ...
    const input = new import_obsidian35.TextComponent(valuesListNotePathContainer);
    input.inputEl.addClass("full-width");
    input.inputEl.addClass("with-label");
    new FileSuggest(
      input.inputEl,
  ...
  createValueContainer(parentNode, key) {
    ...
    const input = new import_obsidian35.TextComponent(valueContainer);
    input.inputEl.addClass("full-width");
    ...
    const valueRemoveButton = new import_obsidian35.ButtonComponent(valueContainer);
    ...
    if (key != Object.keys(this.field.options)[0]) {
      const valueUpgradeButton = new import_obsidian35.ButtonComponent(valueContainer);
      (0, import_obsidian35.setIcon)(valueUpgradeButton.buttonEl, "up-chevron-glyph");
  ...
  createValuesFromDVQueryContainer(parentContainer) {
    ...
    const valuesFromDVQuery = new import_obsidian35.TextAreaComponent(valuesFromDVQueryContainer);
  ...
  createSettingContainer(container, plugin, location) {
    ...
    const sourceType = new import_obsidian35.DropdownComponent(sourceTypeContainer);
...
var SelectField = class extends AbstractListBasedField {
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian36.setIcon)(dropDownButton, "down-chevron-glyph");
    const file = this.plugin.app.vault.getAbstractFileByPath(p["file"]["path"]);
...
var CycleField = class extends AbstractListBasedField {
  ...
  createSettingContainer(container, plugin, location) {
    ...
    const allowNullToggler = new import_obsidian37.ToggleComponent(allowNullValueContainer);
  ...
  createDvField(dv, p, fieldContainer, attrs = {}) {
    ...
    (0, import_obsidian37.setIcon)(cycleBtn, FieldIcon["Cycle" /* Cycle */]);
    if (!((_a = attrs == null ? void 0 : attrs.options) == null ? void 0 : _a.alwaysOn)) {
      cycleBtn.hide();
      spacer.show();
      fieldContainer.onmouseover = () => {
...
var DateModal = class extends BaseModal {
  ...
  buildPath(value) {
    ...
    while (!next.done) {
      if (next.value.groups) {
        ...
        this.pathTemplateItems[pattern] = (0, import_obsidian41.moment)(value).format(pattern);
      }
      next = tP.next();
    }
  ...
  async save() {
    ...
    if (this.plugin.app.plugins.enabledPlugins.has("nldates-obsidian")) {
      try {
        ...
        newValue = parsedDate.date ? parsedDate.moment : (0, import_obsidian41.moment)(`${this.value}`, this.format);
      } catch (error) {
  ...
  buildClearBtn(container) {
    const clearBtn = new import_obsidian40.ButtonComponent(container);
...
var ObjectListField = class extends FieldManager2 {
  ...
  */
  ...
  createDvField(dv, p, fieldContainer, attrs) {
    ...
    editBtn.onclick = async () => {
      const file = this.plugin.app.vault.getAbstractFileByPath(p["file"]["path"]);
      const _eF = file instanceof import_obsidian51.TFile && file.extension == "md" && await Note.getExistingFieldForIndexedPath(this.plugin, file, this.field.id);
      if (_eF)
        this.createAndOpenFieldModal(file, this.field.name, _eF, _eF.indexedPath);
    };
  }
...
var FileClassQuery = class {
  ...
  getResults(api) {
    try {
      ...
      new import_obsidian58.Notice(` for <${this.name}>. Check your settings`);
      return [];
    }
  }
  matchFile(file) {
...
var FieldIndex = class extends FieldIndexBuilder {
  ...
  async getFilesLookupAndFormulaFieldsExists(file) {
    ...
    [...this.filesFields.entries()].forEach(([filePath, fields]) => {
      const dvFields = fields.filter((f) => f.isRoot() && ["Lookup" /* Lookup */, "Formula" /* Formula */].includes(f.type) && f.name in this.dv.api.page(filePath));
...
var IndexStatus = class extends import_obsidian62.Component {
  ...
  checkForUpdate(view) {
    if (view && view instanceof import_obsidian62.FileView && view.file) {
...
var MetadataMenuSettingTab = class extends import_obsidian69.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian69.Setting(globalSettings).setName("Scope").setDesc("Index fields in frontmatter only or in the whole note (if you use dataview inline fields). Indexing full notes could cause some latencies in vaults with large files").addDropdown((cb) => {
    ...
    const showFileClassSelectInModal = new import_obsidian69.Setting(classFilesSettings).setName("Fileclass Select").setDesc("Show fileclass select option in note fields modals").addToggle((cb) => {
...
var updateProps = async (plugin, view) => {
  if (!(view instanceof import_obsidian71.MarkdownView) || !(view.file instanceof import_obsidian71.TFile) || view.file === void 0)
    return;
  const file = view.file;
  ...
  view.metadataEditor.rendered.forEach((item) => {
    ...
    if (plugin.settings.enableProperties) {
      ...
      const btn = new import_obsidian71.ButtonComponent(btnContainer);
  ...
  fileClasses == null ? void 0 : fileClasses.forEach((fileClass) => {
    const addFieldButton = new import_obsidian71.ButtonComponent(fileClassButtonsContainer);
...
async function updatePropertiesSection(plugin) {
  ...
  const currentView = plugin.app.workspace.getActiveViewOfType(import_obsidian71.MarkdownView);
...
var FileClassFolderButton = class extends import_obsidian72.Component {
  constructor(plugin) {
    super();
    this.plugin = plugin;
    this.addButton();
  ...
  addButton() {
    ...
    if (!container.findAll(".fileClass-add-button").length) {
      ...
      (0, import_obsidian72.setIcon)(addBtn, "plus-circle");
      addBtn.onclick = (e) => {
        e.preventDefault();
...
// main.ts
var MetadataMenu = class extends import_obsidian73.Plugin {
  constructor() {
    super(...arguments);
    this.presetFields = [];
...
------------------------------------------`,
    ...
    this.registerEvent(
      this.app.workspace.on("metadata-menu:indexed", () => {
        ...
        const currentView = this.app.workspace.getActiveViewOfType(import_obsidian73.MarkdownView);
        if (currentView)
